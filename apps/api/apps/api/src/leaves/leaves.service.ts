...
export class LeavesService {
  ...
  async create(dto: CreateLeaveDto): Promise<unknown> {
    ...
    const leaveWithEmployee = leave as any;
    if (leaveWithEmployee.employee.manager) {
      const managerUser = await this.prisma.user.findFirst({
        where: { employee: { id: leaveWithEmployee.employee.managerId! } },
      });
      if (managerUser) {
        await this.prisma.notification.create({
          data: {
            ...
            body: `${leaveWithEmployee.employee.firstName} ${leaveWithEmployee.employee.lastName} a soumis une demande de congé du ${startDate.toLocaleDateString('fr-FR')} au ${endDate.toLocaleDateString('fr-FR')} (${days} jour(s)).`,
